...
export default defineEventHandler(async (event) => {
  try {
    ...
    const settings: any = await $fetch('/api/frontend/config', { credentials: 'include' }).catch(() => null)
    ...
    const msgRes: any = await $fetch('/api/messages', { query: { page: 1, pageSize: 20 }, credentials: 'include' }).catch(() => null)
